...
const { useStore, dispatch } = StockoutStore;
...
const OptionCol = (props: IOptionColProps) => {
  ...
  const onView = () => {
    hashHistory.push(`/repertory/stockout?batch=${record.id}`);
  };
  return (
    <>
      <Tooltip title="查看详情">
...
const columns: Array<ColumnProps<Stockout>> = [
  {
    title: '唯一ID',
    dataIndex: 'id'
  },
  ...
  },
  {
    title: '状态',
    dataIndex: 'status',
    render: (status: stockoutStatus) => {
      const statusText =
        {
          [stockoutStatus.ACTIVE]: {
            color: 'green',
            text: '有效'
          },
          [stockoutStatus.INACTIVE]: {
            color: '#f50',
            text: '无效'
          }
        }[status] || {};
...
export const Tables = () => {
  ...
  const onShowCreate = () => {
    hashHistory.push(`/repertory/stockout?batch`);
  };
  const getData = async (num: number = 5) => {
    setLoding(true);
    try {
      await dispatch('getList', num);
      message.success('获取出库单数据成功');
    } catch (error) {
